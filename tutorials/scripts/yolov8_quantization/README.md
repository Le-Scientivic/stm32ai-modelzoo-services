...
- The script supports different formats for input/output, e.g. float, int8 or uint8.

## Installation
- The Python script have been validated in a python v3.12.9 environment.
...
- model:
  ...
  - input_shape: [256, 256, 3] => specify the input shape of the model (height, width, channels).
- quantization:
  - fake: False => If True, will do fake quantization (just for performance evaluation and not accuracy evaluation).
             If False, will do real quantization using the calibration dataset.
