...
from image_classification.pt.src.datasets.food101 import Food101

__all__ = ['get_food101']

@DATASET_WRAPPER_REGISTRY.register(framework='torch', dataset_name='food101', use_case="image_classification")
def get_food101(cfg):
...
def create_quantization_dataset(args):
    ...
    if args.get("quantization_path"):
        ...
        data_root = args["data_dir"]
        LOGGER.info(f"Loading quantization data from training data at: {data_root}/food-101")
        dataset_train = Food101(
            root=data_root,
            split="train",
            download=args["download"],
