...
from image_classification.pt.src.datasets.flowers102 import Flowers102

__all__ = ['get_flowers102']

@DATASET_WRAPPER_REGISTRY.register(framework='torch', dataset_name='flowers102', use_case="image_classification")
def get_flowers102(cfg):
    args = prepare_kwargs_for_dataloader(cfg)
    # args is dict after this point
    args['augmentation_mode'] = 'imagenet' # is this right ? Its based on old code
...
def create_training_dataset(args):
    ...
    if args["augmentation_mode"] == "imagenet":
        ...
        default_train_transforms, _ = get_vanilla_transforms(
            args["img_size"],
            hflip=args["hflip"],
...
def create_validation_dataset(args):
    ...
    if args["augmentation_mode"] == "imagenet":
        ...
        _, default_val_transforms = get_vanilla_transforms(
            args["img_size"],
            hflip=args["hflip"],
            jitter=args["color_jitter"],
            mean=args["mean"] or IMAGENET_DEFAULT_MEAN,
...
def create_test_dataset(args):
    ...
    if args["augmentation_mode"] == "imagenet":
        ...
        _, default_test_transforms = get_vanilla_transforms(
            args["img_size"],
            hflip=args["hflip"],
            jitter=args["color_jitter"],
            mean=args["mean"] or IMAGENET_DEFAULT_MEAN,
...
def create_quantization_dataset(args):
    ...
    elif args.get("data_dir"):
        data_root = args["data_dir"]
        LOGGER.info(f"Loading quantization data from training data at: {data_root}/flowers-102")
        dataset_train = Flowers102(
            root=data_root,
            split='train',
            download=args["download"],
